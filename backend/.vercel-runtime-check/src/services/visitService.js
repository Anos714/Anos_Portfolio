...
export const recordVisit = async ({ ip, userAgent, path, referrer }) => {
    const db = getDb();
    ...
    const [uniqueVisitorsResult, todayUniqueVisitorsResult, pageViewsResult] = await Promise.all([
        db
            .select({ value: sql `count(distinct ${visits.visitorId})` })
            .from(visits),
        db
            .select({ value: count() })
            .from(visits)
